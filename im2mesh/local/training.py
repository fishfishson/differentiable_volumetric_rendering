...
from im2mesh.common import (
    check_weights, get_tensor_values,
    transform_to_world, transform_to_camera_space,
    arange_pixels, sample_patch_points,
...
class Trainer(BaseTrainer):
    def __init__(self, model, optimizer, device=None, vis_dir=None,
        ...
        self.lambda_deform = lambda_normal
    ...
    def compute_loss(self, data, eval_mode=False, it=None):
        ...
        # with masks
        (p_world_hat, rgb_pred, mask_pred, p_world_hat_sparse, mask_pred_sparse, deform) = self.model(
            p, inputs, camera_mat, world_mat, scale_mat, sparse_depth, self.lambda_deform != 0)
        ...
        self.calc_deform_loss(deform, batch_size, loss, eval_mode)
    ...
    def calc_deform_loss(self, deform, batch_size, loss={}, eval_mode=False):
        ...
        Args:
            ...
            eval_mode (bool): whether to use eval mode
        '''
        if self.lambda_deform != 0:
            deform_loss = torch.norm(deform[0] - deform[1], dim=-1).sum() *\
                self.lambda_deform / batch_size
            loss['loss'] += deform_loss
            loss['deform_loss'] = deform_loss
            if eval_mode:
                deform_loss_eval = torch.norm(
                    deform[0] - deform[1], dim=-1).mean() * \
                    self.lambda_deform
                loss['deform_loss_eval'] = deform_loss_eval
